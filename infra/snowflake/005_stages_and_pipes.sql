...
CREATE PIPE IF NOT EXISTS MARKET_DATA.SILVER_EVENTS.PIPE_CURVE_EVENTS
...
AS
COPY INTO MARKET_DATA.SILVER_EVENTS.CURVE_EVENTS (
    event_id,
    provider,
    instrument,
    instrument_type,
...
FROM (
    SELECT
        ...
        $1
    FROM @MARKET_DATA.SILVER_EVENTS.MDRP_SILVER_STAGE
)
FILE_FORMAT = (FORMAT_NAME = 'MARKET_DATA.SILVER_EVENTS.JSON_STRIP_ARRAY');

-- ── DLQ_EVENTS pipe ──────────────────────────────────────────────────────────

CREATE PIPE IF NOT EXISTS MARKET_DATA.SILVER_EVENTS.PIPE_DLQ_EVENTS
  AUTO_INGEST = FALSE
  COMMENT     = 'Snowpipe for SILVER_EVENTS.DLQ_EVENTS — triggered by silver-loader'
AS
COPY INTO MARKET_DATA.SILVER_EVENTS.DLQ_EVENTS (
    dlq_event_id,
    original_event_id,
    provider,
    instrument,
...
CREATE PIPE IF NOT EXISTS MARKET_DATA.GOLD_CURVES.PIPE_FORWARD_CURVE_SNAPSHOTS
  AUTO_INGEST = FALSE
  COMMENT     = 'Snowpipe for GOLD_CURVES.FORWARD_CURVE_SNAPSHOTS — triggered by gold-loader'
AS
COPY INTO MARKET_DATA.GOLD_CURVES.FORWARD_CURVE_SNAPSHOTS (
    snapshot_id,
    provider,
    instrument,
    instrument_type,
...
FROM (
    SELECT
        ...
        $1:batch_id::VARCHAR(36),
        $1:trace_id::VARCHAR(36)
    FROM @MARKET_DATA.GOLD_CURVES.MDRP_GOLD_STAGE
)
FILE_FORMAT = (FORMAT_NAME = 'MARKET_DATA.GOLD_CURVES.JSON_STRIP_ARRAY');

-- ── PROVIDER_QUALITY_HISTORY pipe ────────────────────────────────────────────

CREATE PIPE IF NOT EXISTS MARKET_DATA.GOLD_CURVES.PIPE_PROVIDER_QUALITY_HISTORY
  AUTO_INGEST = FALSE
  COMMENT     = 'Snowpipe for GOLD_CURVES.PROVIDER_QUALITY_HISTORY — triggered by gold-loader'
AS
COPY INTO MARKET_DATA.GOLD_CURVES.PROVIDER_QUALITY_HISTORY (
    quality_history_id,
    provider,
    quality_score,
    events_count,
