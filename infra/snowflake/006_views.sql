...
WITH ranked AS (
    ...
    FROM MDRP.GOLD_CURVES.FORWARD_CURVE_SNAPSHOTS
)
SELECT
    snapshot_id,
    provider,
...
WITH daily AS (
    SELECT
        ...
        CASE
            ...
            ELSE NULL
        END                                      AS dlq_rate_pct
    FROM MDRP.GOLD_CURVES.PROVIDER_QUALITY_HISTORY
...
FROM MDRP.SILVER_EVENTS.DLQ_EVENTS
...
GRANT SELECT ON VIEW MDRP.GOLD_CURVES.V_LATEST_CURVES          TO ROLE MDRP_READER;
GRANT SELECT ON VIEW MDRP.GOLD_CURVES.V_PROVIDER_QUALITY_SUMMARY TO ROLE MDRP_READER;
GRANT SELECT ON VIEW MDRP.SILVER_EVENTS.V_DLQ_SUMMARY          TO ROLE MDRP_READER;
GRANT SELECT ON VIEW MDRP.GOLD_CURVES.V_CURVE_COMPLETENESS     TO ROLE MDRP_READER;

GRANT SELECT ON VIEW MDRP.GOLD_CURVES.V_LATEST_CURVES          TO ROLE MDRP_LOADER;
GRANT SELECT ON VIEW MDRP.GOLD_CURVES.V_PROVIDER_QUALITY_SUMMARY TO ROLE MDRP_LOADER;
GRANT SELECT ON VIEW MDRP.SILVER_EVENTS.V_DLQ_SUMMARY          TO ROLE MDRP_LOADER;
GRANT SELECT ON VIEW MDRP.GOLD_CURVES.V_CURVE_COMPLETENESS     TO ROLE MDRP_LOADER;
